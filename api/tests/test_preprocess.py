...
from api.inference.preprocess import filter_data, filter_hentai
...
class TestFilterHentai:
    ...
    def test_removes_hentai_anime(self):
        ...
        ratings = pd.DataFrame({"anime_id": [1, 2, 3], "user_id": [100, 100, 100], "score": [8, 9, 7]})
    ...
    def test_case_insensitive_filter(self):
        ...
        anime = pd.DataFrame({"anime_id": [1, 2], "name": ["Safe", "Unsafe"], "genre": ["Action", "HENTAI"]})
        ratings = pd.DataFrame({"anime_id": [1, 2], "user_id": [100, 100], "score": [8, 9]})
    ...
    def test_keeps_all_safe_anime(self):
        ...
        anime = pd.DataFrame(
            {"anime_id": [1, 2, 3], "name": ["Anime A", "Anime B", "Anime C"], "genre": ["Action", "Comedy", "Drama"]}
        )
        ratings = pd.DataFrame({"anime_id": [1, 2, 3], "user_id": [100, 100, 100], "score": [8, 9, 7]})
    ...
    def test_checks_all_columns(self):
        ...
        anime = pd.DataFrame(
            {"anime_id": [1, 2], "name": ["Hentai Prince", "Normal Anime"], "genre": ["Comedy", "Action"]}
        )
        ratings = pd.DataFrame({"anime_id": [1, 2], "user_id": [100, 100], "score": [8, 9]})
...
class TestFilterData:
    ...
    def test_filters_by_min_likes_anime(self, sample_ratings):
        ...
        active_anime, _ = filter_data(sample_ratings, min_likes_user=1, min_likes_anime=3)
    ...
    def test_filters_by_min_likes_user(self):
        ...
        _, filtered_df = filter_data(ratings, min_likes_user=2, min_likes_anime=1)
    ...
    def test_returns_integer_anime_ids(self, sample_ratings):
        ...
        active_anime, _ = filter_data(sample_ratings, min_likes_user=1, min_likes_anime=1)
        assert all(isinstance(aid, int) for aid in active_anime)

    def test_empty_after_filtering(self):
        """Handles case where all data is filtered out."""
        ratings = pd.DataFrame({"user_id": [1], "anime_id": [10], "score": [5], "status": ["Completed"]})
        active_anime, filtered = filter_data(ratings, min_likes_user=100, min_likes_anime=100)
