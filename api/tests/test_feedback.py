...
def test_feedback(client, test_user):
    ...
    feedback_items = [
        ...
        RecommendationFeedback(user_id=test_user.id, anime_id=2, action="dismissed", recorded_at=datetime.now(UTC)),
        RecommendationFeedback(user_id=test_user.id, anime_id=3, action="watched", recorded_at=datetime.now(UTC)),
    ]
...
class TestSubmitFeedback:
    ...
    def test_submit_feedback_authenticated(self, client, auth_headers):
        ...
        response = client.post(
            "/api/v1/feedback",
            json={"anime_id": 1, "action": "favorited", "recommendation_request_id": "req123"},
    ...
    def test_submit_feedback_anonymous(self, client):
        ...
        response = client.post("/api/v1/feedback", json={"anime_id": 1, "action": "favorited"})
        assert response.status_code == 201
        data = response.json()
        assert data["anime_id"] == 1
        assert data["action"] == "favorited"
    ...
    def test_submit_feedback_all_actions(self, client, auth_headers):
        ...
        for i, action in enumerate(actions):
            response = client.post("/api/v1/feedback", json={"anime_id": i + 1, "action": action}, headers=auth_headers)
            assert response.status_code == 201
    ...
    def test_submit_feedback_invalid_action(self, client, auth_headers):
        ...
        response = client.post(
            "/api/v1/feedback", json={"anime_id": 1, "action": "invalid_action"}, headers=auth_headers
        )
        assert response.status_code == 422

    def test_submit_feedback_invalid_anime_id(self, client, auth_headers):
        """Submit feedback with invalid anime_id."""
        response = client.post("/api/v1/feedback", json={"anime_id": -1, "action": "favorited"}, headers=auth_headers)
        assert response.status_code == 422
...
class TestBulkFeedback:
    ...
    def test_submit_bulk_feedback_anonymous(self, client):
        ...
        response = client.post(
            "/api/v1/feedback/bulk",
            json={"feedback_items": [{"anime_id": 1, "action": "favorited"}, {"anime_id": 2, "action": "dismissed"}]},
        )
        assert response.status_code == 201
    ...
    def test_submit_bulk_feedback_empty_list(self, client, auth_headers):
        ...
        response = client.post("/api/v1/feedback/bulk", json={"feedback_items": []}, headers=auth_headers)
        assert response.status_code == 422
...
class TestFeedbackStats:
    ...
    def test_get_feedback_stats_with_days_filter(self, client, auth_headers, test_feedback):
        ...
        response = client.get("/api/v1/feedback/stats?days=7", headers=auth_headers)
        assert response.status_code == 200
...
class TestFeedbackHistory:
    ...
    def test_get_feedback_history_with_pagination(self, client, auth_headers, test_feedback):
        ...
        response = client.get("/api/v1/feedback/history?limit=2&offset=0", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_feedback_history_filter_by_action(self, client, auth_headers, test_feedback):
        ...
        response = client.get("/api/v1/feedback/history?action=favorited", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
