...
class TestAuthRegister:
    ...
    def test_register_invalid_email(self, client):
        ...
        response = client.post("/api/v1/auth/register", json={"email": "not-an-email", "password": "validpassword123"})
        assert response.status_code == 422  # Validation error
...
class TestAuthLogin:
    ...
    def test_login_nonexistent_user(self, client):
        ...
        response = client.post(
            "/api/v1/auth/login", data={"username": "nobody@example.com", "password": "anypassword123"}
        )
        assert response.status_code == 401
...
class TestAuthMe:
    ...
    def test_get_me_authenticated(self, client, test_user):
        ...
        login_response = client.post(
            "/api/v1/auth/login", data={"username": "test@example.com", "password": "testpassword123"}
        )
